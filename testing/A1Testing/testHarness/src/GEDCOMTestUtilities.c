...
bool _tListEqual(List testList, List refList, bool (*isEqual)(const void* test, const void* ref)){
    ...
    for (Node* ptr = refList.head; ptr != NULL; ptr = ptr->next){
        ...
        if (!_tContains(testList, ptr->data, isEqual)){
           ...
           // printf("Test list does not contain reference object\n");
           // printf("Reference Object: \n");
           // printf("\n%s\n", testList.printData(ptr->data));
           // printf("Test List: \n");

          //  for (Node* ptr1 = testList.head; ptr1 != NULL; ptr1 = ptr1->next){
          //      printf("\n%s\n", testList.printData(ptr1->data));
          //  }
    ...
    for (Node* ptr = testList.head; ptr != NULL; ptr = ptr->next){
        ...
        if (!_tContains(refList, ptr->data, isEqual)){
            //printf("Reference list does not contain test object\n");
            return false;
        }
    }
...
bool _tEventEqual(const void* testObj, const void* refObj){
    ...
    if (strcmp(testEvt->type, refEvt->type ) != 0){
       // printf("Events are not equal because type is not equal\n");
        return false;
    }
...
//        printf("Events are not equal because place is not equal\n");
...
bool _tIndEqual(const void* testObj, const void* refObj){
    ...
    if (testObj == NULL || refObj == NULL){
        //printf("Individuals not equal because one object is NULL\n");
        return false;
    }
    ...
    if (refInd->givenName != NULL && testInd->givenName != NULL && (strcmp(testInd->givenName, refInd->givenName ) != 0)){
         //printf("Individuals not equal because given names are not the same\n");
         //printf("Test name 1: %s Reference name : %s\n", testInd->givenName, refInd->givenName);
        return false;
    }
    
    if (refInd->surname != NULL && testInd->surname != NULL && (strcmp(testInd->surname, refInd->surname ) != 0)){
         //printf("Individuals not equal because surnames are not the same\n");        
        return false;
    }
...
//    }
    ...
    if (!_tListEqual(testInd->events, refInd->events, &_tEventEqual)){
       //  printf("Individuals not equal because events are not the same\n");        
    ...
    if (!_tListEqual(testInd->families, refInd->families, &_tFamEqualShallow)){
         ...
         //printf("Individuals not equal because family lists are not the same\n");       
...
bool _tFamEqual(const void* testObj, const void* refObj){
    ...
    if (testObj == NULL || refObj == NULL){
       // printf("Families are not equal because one of the families are NULL\n");    
    ...
    if (!_tPtrCmp(testFam->husband, refFam->husband) && (!_tIndEqualShallow(testFam->husband, refFam->husband))){
      //  printf("Families are not equal because husbands are not the same\n");    
        return false;
    }
   
    if (!_tPtrCmp(testFam->wife, refFam->wife) && (!_tIndEqualShallow(testFam->wife, refFam->wife))){
      //  printf("Families are not equal because wivess are not the same\n");    
        return false;
    }
...
//    }
    ...
    if (!_tListEqual(testFam->events, refFam->events, &_tEventEqual)){
       // printf("Families are not equal because events are not the same\n");    
        return false;
    }
    
    //Compare children - shallow!
    if (!_tListEqual(testFam->children, refFam->children, &_tIndEqualShallow)){
        //printf("Families are not equal because children are not the same\n");    

        return false;
    }
    
    //printf("Families are the same\n");
    return true;
}
