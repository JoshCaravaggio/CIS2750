...
char* indToJSON(const Individual* ind){
	...
	currentLength = strlen(JSONstring);
	JSONstring = realloc(JSONstring, sizeof(char)*(currentLength + 30));
	strcat(JSONstring, "\",\"sex\":\"");
	char* individualSex;
	bool sexFound = false;

	for(Node * node = ind->otherFields.head; node!=NULL; node  = node->next){

		Field * field = (Field*)node->data;

		if(strcmp(field->tag, "SEX")==0){
			//printf("Found sex for %s %s\n",ind->givenName, ind->surname );
			individualSex = field->value;
			sexFound = true;			
		}
	...
	if(sexFound == true){
		...
		JSONstring = realloc(JSONstring, sizeof(char)*(currentLength + strlen(individualSex) + 20));		
		strcat(JSONstring, individualSex);		
		strcat(JSONstring, "\"}");		

	}else{

		JSONstring = realloc(JSONstring, sizeof(char)*(currentLength +20));				
		strcat(JSONstring, "\"}");			
	...
	//int familySize = getFamilySize(ind);
...
GEDCOMobject* JSONtoGEDCOM(const char* str){
	...
	GEDCOMobject* newObj = calloc(sizeof(GEDCOMobject),1);
	newObj->header =NULL;
...
char* GEDCOMtoJSON(char* fileName){
	...
	FILE * fp;	
	char* JSONstring = calloc(sizeof(char)*40, 1);
	...
	//	printf("Type not correct: %d\n", error.type);
		JSONstring = realloc(JSONstring, sizeof(char)* 40);		
		strcpy(JSONstring, "{\"fileName\":\"INVALID_FILE\"}");
		return JSONstring;

	}

	JSONstring  = realloc(JSONstring, sizeof(char)*(strlen(fileName) + 40));
	sprintf(JSONstring, "{\"fileName\":\"%s\",", fileName);
	currentLength = strlen(JSONstring);

	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + strlen(obj->header->source) + 20));
	strcat(JSONstring, "\"source\":\"");
	strcat(JSONstring, obj->header->source);
	strcat(JSONstring, "\",");
	currentLength = strlen(JSONstring);

	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + 40));
	strcat(JSONstring, "\"version\":5.5,");
	currentLength = strlen(JSONstring);
	...
	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + 20));
	strcat(JSONstring, encodeCharSet(obj->header->encoding));
	strcat(JSONstring, "\",");
	currentLength = strlen(JSONstring);

	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + 40));
	strcat(JSONstring, "\"submitterName\":\"");
	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + strlen(obj->submitter->submitterName) + 20));	
	...
	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + 40));
	strcat(JSONstring, "\"submitterAddress\":\"");
	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + strlen(obj->submitter->address) + 20));	

	strcat(JSONstring, obj->submitter->address);
	strcat(JSONstring, "\",");
	currentLength = strlen(JSONstring);

	int length = (obj->individuals).length;
	char* tempLength = calloc(sizeof(char),10);
	sprintf(tempLength, "%d", length);

	JSONstring  = realloc(JSONstring, sizeof(char)*(currentLength + 40));
	strcat(JSONstring, "\"numIndividuals\":");
	strcat(JSONstring, tempLength);
	strcat(JSONstring, ",");
	currentLength = strlen(JSONstring);	
	free(tempLength);

	length = (obj->families).length;
	tempLength = calloc(sizeof(char),10);
	sprintf(tempLength, "%d", length);
...
char* getIndividualsFromGEDCOM(char* fileName){
	...
	if(error.type != OK){
		...
		JSONstring = realloc(JSONstring, sizeof(char)* 40);		
		strcpy(JSONstring, "{\"data\":\"INVALID_FILE\"}");
		return JSONstring;

	}

	List individualsList = obj->individuals;
	char* indListJson = iListToJSON(individualsList);
	currentLength = strlen(JSONstring) + strlen(indListJson);
	JSONstring = realloc(JSONstring, sizeof(char)*(currentLength+20));
	strcpy(JSONstring, indListJson);
...
char * createNewGEDCOM(char* JSONstring, char* fileName){
	...
	GEDCOMobject* newObj = JSONtoGEDCOM(JSONstring);
	GEDCOMerror createError;
	createError.type = validateGEDCOM(newObj);
	GEDCOMerror writeError;

	char* fileNameCopy = calloc(sizeof(char)*120,1);
	strcpy(fileNameCopy, "./uploads/");
	strcat(fileNameCopy, fileName);

	if(createError.type == OK){

		writeError = writeGEDCOM(fileNameCopy, newObj);
		response = realloc(response, sizeof(char) * 60);
		strcat(response, printError(writeError));
